...
class DFGExtractorV1_12:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        def _add_use_edge(var: str, role: str, dst_sid: int):
            ...
            has_lower = (
                _as_int(g_var.get("lower", 0))
                | _as_int(g_all.get("lower", 0))
                | _as_int(g_agg.get("lower", 0))
            )
            has_upper = (
                _as_int(g_var.get("upper", 0))
                | _as_int(g_all.get("upper", 0))
                | _as_int(g_agg.get("upper", 0))
            )
            upper_norm = max(
        ...
        for row in self.nodes:
            ...
            if node_type == "AssignmentExpression" and isinstance(orig, dict):
                ...
                if isinstance(rhs_node, dict) and isinstance(
                    self._find_first_call_node(rhs_node), dict
                ):
            ...
            if (
                ...
                print(
                    f"(특수) 문장-수준 호출 노드가 ArgList만 가리키는 경우: 여기서 호출 자체 처리 : node = {code}"
                )
            ...
            if node_type == "AssignmentExpression" and isinstance(orig, dict):
                ...
                if (
                    isinstance(lhs, dict)
                    and lhs.get("nodeType") == "ArraySubscriptExpression"
                ):
                    kids = lhs.get("children") or []
                    idx = kids[1] if len(kids) > 1 else None
                    if isinstance(idx, dict):
                        for v in self._idents_from_ast_node(
                ...
                lhs_is_object_base = (
                    False  # buffer[i], s.field, data(식별자) 같은 '객체에 쓰기'(DEF)
                )
                ...
                if isinstance(lhs, dict):
                    ...
                    if lhs_nt == "ArraySubscriptExpression":
                        ...
                        if isinstance(base, dict):
                            ...
                            if (
                                not lhs_base_name
                                and base.get("nodeType") == "Identifier"
                            ):
                                lhs_base_name = base.get("name")

                        print(
                            f"ArraySubscriptExpression - code: {code}, lhs_base_name : {lhs_base_name}"
                        )

                        # 포인터 베이스 여부
                        if (
                            isinstance(base, dict)
                            and base.get("nodeType") == "PointerDereference"
                        ):
                            lhs_is_pointer_base = True

                        print(
                            f"ArraySubscriptExpression - code: {code}, lhs_is_pointer_base : {lhs_is_pointer_base}"
                        )

                        # 객체 베이스 여부: 배열 변수/필드 등 (pointer_vars에 없고 식별자/MemberAccess면 객체 취급)
                        if not lhs_is_pointer_base:
                            if isinstance(base, dict) and base.get("nodeType") in {
                                "Identifier",
                                "MemberAccess",
                            }:
                    ...
                    if lhs_is_object_base:
                        ...
                        if lhs_nt in {"ArraySubscriptExpression", "MemberAccess"}:
                            ...
                            if lhs_nt == "ArraySubscriptExpression":
                                ...
                                idx = kids[1] if len(kids) > 1 else None
                                if isinstance(idx, dict):
                                    idx_vars = self._idents_from_ast_node(
                                        idx, skip_sizeof=True, skip_callee=True
                                    )
                ...
                try:
                    ...
                    rhs_call = (
                        self._find_first_call_node(rhs)
                ...
                try:
                    ...
                    calln = (
                        self._find_first_call_node(rhs2)
                        if isinstance(rhs2, dict)
                        else None
                    )
            ...
            if isinstance(orig, dict):
                ...
                if node_type == "AssignmentExpression":
                    ...
                    if isinstance(rhs, dict) and isinstance(
                        self._find_first_call_node(rhs), dict
                    ):
                        scan_node = lhs if isinstance(lhs, dict) else orig
                for t in self._idents_from_ast_node(
                    scan_node, skip_sizeof=True, skip_callee=True
                ):
        ...
        for meta in self.nodes:
            ...
            # Attach orig_id from flattened AST row if available
            flat_row = self._find_ast_row_by_sid(sid)
    ...
    def _call_arg_uses_ast(
        ...
        for a in arg_nodes or []:
            if isinstance(a, dict) and a.get("nodeType") == "ArraySubscriptExpression":
                ...
                if isinstance(idx_node, dict):
                    for t in self._idents_from_ast_node(
                        idx_node, skip_sizeof=True, skip_callee=True
                    ):
                        _emit(t, "index")
                        index_vars.add(t)
        ...
        if size_pos is not None and 0 <= size_pos < len(arg_nodes or []):
            ...
            if isinstance(size_arg, dict):
                for t in self._idents_from_ast_node(
                    size_arg, skip_sizeof=True, skip_callee=True
                ):
                    _emit(t, "size")
                    size_vars.add(t)
    ...
    def _build_guard_map(self):
        ...
        pc_edges = (
            ast_res.get("edges_ast_pc") or getattr(self, "edges_ast_pc", []) or []
        )
        sb_edges = (
            ast_res.get("edges_ast_sb") or getattr(self, "edges_ast_sb", []) or []
        )
        grd_edges = (
            ast_res.get("edges_ast_guard") or getattr(self, "edges_ast_guard", []) or []
        )
        ...
        def _get_cond_ast_fallback(nt: str, ast_node: dict):
            kids = (
                (ast_node.get("children") or []) if isinstance(ast_node, dict) else []
            )
            if nt == "IfStatement":
                return kids[0] if len(kids) >= 1 and isinstance(kids[0], dict) else None
            if nt == "ForStatement":
