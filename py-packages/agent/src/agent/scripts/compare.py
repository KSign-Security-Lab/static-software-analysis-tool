    ...
    if isinstance(schema, list):
        ...
        if len(schema) == 1:
            ...
            for i, (item1, item2) in enumerate(zip(obj1, obj2)):
                sub_path = f"{path}[{i}]"
                if not isinstance(item1, (list, tuple)) or not isinstance(
                    item2, (list, tuple)
                ):
                    raise ValueError(
                        f"Type mismatch at {sub_path}: expected tuple-like list"
                    )
                ...
                for j, sub_schema in enumerate(tuple_schema):
                    compare_by_schema(
                        item1[j], item2[j], sub_schema, f"{sub_path}[{j}]"
                    )
            return True
...
def analyze_differences(
    dfg1: Dict[str, Any], dfg2: Dict[str, Any], file1: str, file2: str
) -> Dict[str, Any]:
    ...
    for key in keys1 & keys2:
        ...
        if (
            isinstance(schema_entry, list)
            and isinstance(dfg1.get(key), list)
            and isinstance(dfg2.get(key), list)
        ):
...
def save_failure_report(
    failure_details: List[Dict], error_types: Dict, dfg1_dir: str, dfg2_dir: str
) -> None:
...
def main(dfg1_dir: str, dfg2_dir: str) -> None:
            ...
            failure_details.append(
                {
                    ...
                    "error_message": str(e),
                    "dfg1_nodes": (
                        len(dfg1.get("nodes", [])) if dfg1 is not None else 0
                    ),
                    "dfg2_nodes": (
                        len(dfg2.get("nodes", [])) if dfg2 is not None else 0
                    ),
                    "dfg1_edges": (
                        len(dfg1.get("edges_dfg", []))
                        if (dfg1 is not None and edges_enabled)
                        else 0
                    ),
                    "dfg2_edges": (
                        len(dfg2.get("edges_dfg", []))
                        if (dfg2 is not None and edges_enabled)
                        else 0
                    ),
                    "analysis": analysis,
                }
    ...
    if failed_comparisons > 0:
        ...
        for i, failure in enumerate(failure_details, 1):
            ...
            print(
                f"    DFG1: {failure['dfg1_nodes']} nodes, {failure['dfg1_edges']} edges"
            )
            print(
                f"    DFG2: {failure['dfg2_nodes']} nodes, {failure['dfg2_edges']} edges"
            )

            if failure.get("analysis"):
                analysis = failure["analysis"]
                if analysis.get("size_differences"):
                    print(f"    Size differences:")
