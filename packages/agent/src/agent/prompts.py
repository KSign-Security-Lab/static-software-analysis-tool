...
고칠 방법이 `anchor_text` 가 차지한 줄만 바꿔서 해결된다면, remediation 의 `replacement`
에 그 줄을 대신할 코드를 그대로 쓰십시오. 사람이 한 번 눌러 그대로 적용하는 코드이므로,
설명이나 diff 기호가 아니라 들여쓰기까지 맞춘 실제 코드여야 합니다. 그 자리만 바꿔서는
안 되는 고침 -- 다른 함수를 손봐야 하거나, 선언을 바꿔야 하거나, 여러 곳을 함께 고쳐야
하는 경우 -- 이라면 `replacement` 는 비워 두고 `detail` 로만 설명하십시오. 반쯤 맞는
코드보다 비어 있는 편이 낫습니다.
...
- access: 인증, 인가, 자격 증명, 키, 권한
- crypto: 해시·암호화·서명·난수의 선택과 사용, IV·nonce·salt, 인증서 검증
...
- search_corpus: 이 코드가 이미 기록된 어떤 약점을 닮았는지. 설명이 아니라 코드를 그대로
  넘기십시오. 이 검사와 무관하게 미리 모아 둔 표본에서 찾으므로, 이 저장소가 스스로에 대해
  하는 말 바깥에서 오는 유일한 근거입니다. 열 개로 재어 보니 CWE 를 여덟 번 맞혔습니다 --
  어느 갈래인지에 대한 단서일 뿐, 이 코드가 실제로 뚫린다는 증거는 아닙니다. 닮았다는 것과
  닿는다는 것은 다르고, 표본은 이 함수로 들어오는 길을 보지 못합니다.
  표본에 붙은 vulnerable/fixed 는 출처 표시이지 판정이 아닙니다. 둘의 점수 차가 재어 보니
  구별할 수 없을 만큼 작으므로, fixed 가 가깝게 나왔다고 해서 이 코드가 고쳐진 것은 아닙니다
