...
from .handlers import HandlerResolver
...
class F2AAnalyzer:
    ...
    def __init__(
        ...
        #: Step 1 lives in its own module; see :class:`~ssat.f2a.handlers.HandlerResolver`.
        self.resolver = HandlerResolver(cpg, self.kb, selection=selection, calculus=self.calculus)
    ...
    @property
    def _selection(self) -> Dict[str, SelectionResult]:
        """Selections made by the resolver, keyed by action.

        Read by :mod:`ssat.f2a.evaluation`; kept as a property so the resolver
        stays the single owner of the cache.
        """
        return self.resolver.selections

    def _resolve_handler(self, action: str) -> SelectionResult:
        """Resolve one action. Delegates to :class:`HandlerResolver`."""
        return self.resolver.resolve(action)

    def _limitation_for(self, action: str, sel: Optional[SelectionResult]) -> str:
        return self.resolver.limitation_for(action, sel)

    def _selection_to_resolution(self, action: str, sel: SelectionResult) -> HandlerResolution:
        return self.resolver.selection_to_resolution(action, sel)
