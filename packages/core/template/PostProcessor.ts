...
export class PostProcessor {
  ...
  public mergeArraySizeAllocation(nodes: TemplateNodes[]): TemplateNodes[] {
    return nodes.map((node) => {
      ...
      for (let i = 0; i < node.children.length; i++) {
        ...
        if (current.nodeType === TemplateNodeTypes.ArrayDeclaration && next.nodeType === TemplateNodeTypes.ArraySizeAllocation) {
  ...
  public updateMemberAccessTypeLength(nodes: TemplateNodes[], cpg: CPGRoot): TemplateNodes[] {
    ...
    const memberAccessChildrenIdentifierNodes = memberAccessNodes
      .flatMap((node) => {
        ...
        return node.children.filter((child): child is IIdentifier => child.nodeType === TemplateNodeTypes.Identifier);
      })
  ...
  private findMemberAccessNodes(nodes: TemplateNodes[]): TemplateNodes[] {
    return nodes.flatMap((node) => {
      if (node.nodeType === TemplateNodeTypes.MemberAccess) {
        return [node];
      }
