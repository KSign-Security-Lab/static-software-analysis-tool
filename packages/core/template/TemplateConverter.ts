...
import { IdentifierToLiteralMap, PredefinedIdentifierTypes } from "./config/Predefined";
...
export class TemplateConverter {
  ...
  /** Convert an array (“forest”) of root nodes into TemplateNodes[], skipping undefined conversions. */
  ...
  private convertedChildren(children: TreeNode[]): TemplateNodes[] {
    return children.map((child) => this.dispatchConvert(child)).filter((child): child is TemplateNodes => child !== undefined);
  }
  ...
  private handleCall(node: TreeNode): CallReturnTypes {
    ...
    return {
      nodeType: STANDARD_LIB_CALLS.has(node.name) ? TemplateNodeTypes.StandardLibCall : TemplateNodeTypes.UserDefinedCall,
      id: Number(node.id) || -999,
      name: node.name,
  ...
  private handleCallOperators(node: TreeNode): CallOperatorsReturnTypes {
    ...
    return {
      ...node,
      children: node.children.map((child) => this.dispatchConvert(child)).filter((child): child is TemplateNodes => child !== undefined),
  ...
  private handleMethod(node: TreeNode): IFunctionDeclaration | IFunctionDefinition | undefined {
    ...
    if (
      ...
      return {
        nodeType: firstBlock && firstBlock.code === "<empty>" ? TemplateNodeTypes.FunctionDeclaration : TemplateNodeTypes.FunctionDefinition,
        id: Number(node.id) || -999,
        name: node.name,
  ...
  private handleSkippedNodes(node: TreeNode): TemplateNodes | undefined {
    return {
      ...node,
      children: node.children.map((child) => this.dispatchConvert(child)).filter((child): child is TemplateNodes => child !== undefined),
    } as unknown as TemplateNodes;
  }
