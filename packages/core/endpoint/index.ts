...
export async function runPythonDFGExtractor(templateData: TemplateNodes[]): Promise<IDFGGraph[]> {
  ...
  for (let i = 0; i < templateFunctions.length; i++) {
    ...
    const pythonCode = [
      ...
      "dfg_dir = os.path.dirname(dfg_extractor_path)",
      "if dfg_dir not in sys.path:",
      "    sys.path.insert(0, dfg_dir)",
      ...
      "DFGExtractor = getattr(dfg_mod, 'DFGExtractorV1_12', None)",
      "if DFGExtractor is None:",
      "    DFGExtractor = getattr(dfg_mod, 'DFGExtractor')",
      "",
      "try:",
      "    # Parse input data",
