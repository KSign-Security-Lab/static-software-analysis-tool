...
class CPGGenerator:
    ...
    async def convert_to_cpg_standalone(
        ...
        try:
            ...
            cpg_export = await self._generate_cpg_from_file(
                str(source_file), representation=representation, export_format=export_format
            )
    ...
    async def _generate_cpg_from_file(
        ...
        # Step 2: joern-export
        export_result = await self._run_joern_export(
            str(tmp_dir), representation=representation, export_format=export_format
        )
    ...
    async def _run_joern_export(
        self, cwd: str, representation: str = "all", export_format: str = "graphson"
    ) -> Dict[str, Any]:
    ...
    async def _generate_cpg_from_file_docker(
        ...
        export_result = await self._run_joern_export_docker(
            container_name, str(workspace_dir), representation=representation, export_format=export_format
        )
        if not export_result["success"]:
            raise RuntimeError(f"joern-export failed: {export_result.get('error', 'Unknown error')}")
    ...
    async def _run_joern_export_docker(
        self, container_name: str, cwd: str, representation: str = "all", export_format: str = "graphson"
    ) -> Dict[str, Any]:
