...
export class DFGBuilder {
  public build(cpg: CPGRoot, ast: IASTResult[], templates: TemplateNodes[]): IDFGGraph[] {
    ...
    const rawDFG: IDFGGraph[] = ast.map(() => ({ nodes, edges }));

    const syncedDfg = new DFGSync().syncPerFile(rawDFG, ast, templates);
