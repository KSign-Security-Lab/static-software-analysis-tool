...
class DFGHandler {
   ...
   */
  async generateDFG(cpgData: CPGRoot, astData: IASTGraph, opts: GenerateDFGOptions = {}): Promise<IDFGGraph> {
    ...
    const rawDFG = dfgBuilder.build(cpgData, astData);

    // If AST data is provided, synchronize the DFG with it
    if (astData) {
      const dfgSync = new DFGSync();

      return dfgSync.sync(rawDFG, astData);
    }
   ...
   */
  async generateDFGFromString(cpgJsonString: string, astData: IASTGraph, opts: GenerateDFGOptions = {}): Promise<IDFGGraph> {
    let cpgData: CPGRoot;
    try {
    ...
    return this.generateDFG(cpgData, astData, opts);
  }
}
