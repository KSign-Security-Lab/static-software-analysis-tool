...
\section{AST Extractor V1.12}

\subsection{개요}
\texttt{ASTExtractorV1\_12} 클래스는 계층적인 템플릿 JSON(함수 수준 AST)을 GNN(Graph Neural Network) 학습에 적합한 평탄화된(Flattened) 문장 수준 그래프 표현으로 변환하는 핵심 모듈이다. 이 모듈은 제어 흐름 관계, 가드(Guard) 조건, 의미적 주석(Semantic Annotations)을 보존하면서 AST 정보를 추출한다.
...
\begin{itemize}
    ...
    \item \textbf{문장 수준 입도}: 표현식(Expression) 수준이 아닌 할당, 선언, 제어문 등 문장(Statement) 수준의 분석이 필요하다.
...
AST Extractor는 계층적 AST를 평탄화하고, 구조 및 제어 흐름을 보존하기 위해 세 가지 유형의 엣지(Parent-Child, Sibling, Guard)를 생성하며, 각 노드에 대해 GNN 입력에 적합한 특징 벡터를 계산함으로써 이러한 문제를 해결한다.


\subsection{실행 흐름}

\subsubsection{초기화 (Initialization)}
\texttt{ASTExtractorV1\_12} 인스턴스 생성 시, 입력된 템플릿 JSON을 순회하여 모든 노드를 ID로 인덱싱하고 결과 컨테이너를 초기화한다.
...
제어문(If, For, While, Switch)은 가드 엣지(Guard Edge)를 통해 제어 노드와 피제어 블록을 연결한다.
...
\begin{itemize}
    \item \textbf{Input}: \texttt{\{ "nodeType": "FunctionDefinition", "children": [...] \}}
    \item \textbf{Output}: \texttt{\{ "nodes": [...], "edges\_ast\_pc": [...], "edges\_ast\_sb": [...], "edges\_ast\_guard": [...] \}}
\end{itemize}

\subsubsection{노드 특징 (Node Features)}
...
\begin{enumerate}
    \item \textbf{Parent-Child (PC)}: 원본 AST의 계층 구조 표현 (엣지 타입 0).
    \item \textbf{Sibling (SB)}: 문장 간의 실행 순서 표현 (엣지 타입 1).
