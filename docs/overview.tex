...
본 프로젝트는 C/C++ 소스 코드의 보안 취약점을 탐지하는 AI 모델 프로토타입을 개발하는 것을 목적으로 한다.
기존 정적 분석 도구들은 높은 False Positive Rate와 문맥 인식의 부재로 인해 실용적 활용에 제약이 있다.
이러한 한계를 극복하기 위하여, 본 연구에서는 코드의 구조적 특성을 활용하는 새로운 접근 방식을 제안한다.

구체적으로, 소스 코드로부터 \abbrAST 를 추출하여 그래프 표현으로 변환하고,
이를 \abbrGNN 을 통한 학습으로 취약점 패턴을 인식한다.
이는 코드의 구조적 관계를 모델링함으로써 문맥을 고려한 정확한 취약점 탐지를 가능하게 한다.
...
\begin{itemize}
\item \textbf{CPG 생성}: Joern을 활용하여 C/C++ 소스 코드를 \abbrCPG 로 변환한다. CPG는 코드의 구문, 제어 흐름, 타입 정보 등을 통합적으로 표현하는 멀티 레이어 그래프 구조이다.
