...
\subsection{개요}
취약점 탐지 모듈은 정적 분석 파이프라인의 후반부에서 동작하는 그래프 신경망(\abbrGNN) 기반의 취약점 탐지 시스템이다. 이 모듈은 앞선 단계에서 생성된 코드의 \abbrAST 표현을 입력으로 받아, 해당 함수가 Vulnerable 인지 혹은 Safe 인지를 분류하도록 학습된 \abbrPyG 기반 AI 모델을 운용한다.

\subsubsection{해결하고자 하는 문제}
기존의 정적 분석 도구는 잠재적 취약점을 일정 수준까지 식별할 수 있으나, 코드의 Semantics와 Context를 충분히 이해하지 못해 다음과 같은 한계가 존재한다.

\begin{itemize}
    \item \textbf{Context Sensitivity 부족}: 동일한 코드 패턴이라도 문맥에 따라 안전할 수도, 취약할 수도 있다.
    \item \textbf{False Positives 증가}: 런타임 제약 조건이나 검증 로직으로 인해 실제로는 안전한 코드임에도 취약점으로 탐지되는 경우가 빈번하다.
    \item \textbf{복잡한 패턴}: 취약점은 단일 구문이 아닌, 여러 코드 요소 간의 복잡한 상호작용으로 인해 발생하는 경우가 많다.
\end{itemize}

AI 모델은 레이블링된 데이터셋을 통해 GNN 모델을 학습시킴으로써 이러한 문제를 해결한다. GNN 아키텍처는 코드를 그래프 구조(구문을 위한 AST)로 표현하고, 노드 및 엣지 속성으로부터 Feature를 자동으로 학습하여 구문적 패턴을 포착하는 데 최적화되어 있다.

\subsubsection{기능 요약}
취약점 탐지 모듈은 다음과 같은 기계 학습 파이프라인 전 과정을 제공한다.

\begin{enumerate}
    \item \textbf{데이터 로딩}: 함수별 AST 그래프가 포함된 JSON 파일을 로드한다.
    \item \textbf{그래프 구축}: JSON 데이터를 PyG의 \texttt{Data} 객체(노드 특징, 엣지 인덱스, 엣지 속성 포함)로 변환한다.
    \item \textbf{모델 학습}: 학습 데이터를 사용하여 함수를 취약(1) 또는 안전(0)으로 분류하는 GNN 모델을 학습한다.
    \item \textbf{모델 평가}: 테스트 데이터를 통해 모델의 정확도, Confusion Matrix, 클래스별 통계 등을 평가한다.
...
\begin{itemize}
\item \textbf{구성 파싱}: \abbrCLI 인자를 통해 \texttt{TrainConfig} 객체를 생성하여 유연한 실험 설정을 지원한다.
...
JSON 파일을 로드하고 PyG 그래프 객체로 변환하는 역할을 담당한다. Pydantic 모델을 사용하여 JSON 스키마를 검증하며, \texttt{juliet\_json\_to\_sample} 변환기를 통해 AST 섹션을 추출하고 그래프를 구축한다.
...
AST 그래프와 가변 메타데이터를 포함하는 샘플들을 하나의 배치로 결합한다. PyG의 \texttt{Batch.from\_data\_list}를 사용하여 효율적인 병렬 처리가 가능한 연결된 그래프(Disjoint Union) 형태를 생성한다.
...
\texttt{evaluate\_model} 및 \texttt{analyze\_sample\_with\_model} 함수를 통해 추론을 수행하고, 정확도, Confidence, 오분류 샘플 등의 상세 분석 결과를 생성한다.
...
본 시스템은 AST 그래프를 처리하는 GNN 모델을 사용한다. \abbrGINE 을 사용하여 그래프를 인코딩하고, 글로벌 풀링 후 선형 분류기를 통해 예측을 수행한다.
...
\begin{itemize}
    \item \textbf{기본 설정}: 에포크 수, 학습률, 배치 크기, 디바이스, GNN 계층 수, 히든 차원 등.
...
취약점 탐지 모듈은 전체 정적 분석 파이프라인의 최종 단계로 통합된다.
...
\begin{enumerate}
    ...
    \item \textbf{GNN 분석}: AI 모델이 JSON을 입력받아 취약점 여부를 판별.
\end{enumerate}
