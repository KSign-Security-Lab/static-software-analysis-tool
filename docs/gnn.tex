...
\subsection{개요}
Agent 모듈은 정적 분석 파이프라인의 후반부에서 동작하는 그래프 신경망(\abbrGNN) 기반의 취약점 탐지 시스템이다. 이 모듈은 앞선 단계에서 생성된 코드의 AST 표현을 입력으로 받아, 해당 함수가 Vulnerable 인지 혹은 Safe 인지를 분류하도록 학습된 \abbrPyG 모델을 운용한다.

\subsubsection{해결하고자 하는 문제}
기존의 정적 분석 도구는 잠재적 취약점을 식별할 수 있으나, 코드의 Semantics와 Context를 완전히 이해하지 못해 다음과 같은 한계가 존재한다.
...
Agent 모듈은 레이블링된 데이터셋을 통해 GNN 모델을 학습시킴으로써 이러한 문제를 해결한다. GNN 아키텍처는 코드를 그래프 구조(구문을 위한 AST)로 표현하고, 노드 및 엣지 속성으로부터 Feature를 자동으로 학습하여 구문적 패턴을 포착하는 데 최적화되어 있다.
